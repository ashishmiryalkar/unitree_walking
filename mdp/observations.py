...
def get_indivisual_feet_distance_wrt_torso(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: Articulation = env.scene[asset_cfg.name]
    torso_pos = asset.data.body_com_pos_w[:, 4, :]
    lft_feet, right_feet = asset.data.body_com_pos_w[:, 20, :] - torso_pos, asset.data.body_com_pos_w[:, 21, :] - torso_pos
    return lft_feet, right_feet

def get_feet_distance_wrt_torso(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    lft_feet, right_feet = get_indivisual_feet_distance_wrt_torso(env, asset_cfg)
    return torch.sqrt(torch.sum((lft_feet - right_feet)**2, dim=-1))
